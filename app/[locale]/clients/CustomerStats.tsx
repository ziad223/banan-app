...
export default function CustomersStats({ stats }: CustomersStatsProps) {
  ...
  const creditUsagePercentage =
    stats.totalCredit > 0
      ? ((Math.abs(stats.totalBalance) / stats.totalCredit) * 100).toFixed(1)
      : '0';
  ...
  const cards = [
    ...
    },
    {
      ...
      description: `${activePercentage}% من إجمالي العملاء`,
      icon: CiViewList,
      color: 'green',
    },
    {
      title: 'إجمالي الرصيد',
      value: `${stats.totalBalance.toLocaleString('ar-EG')} ج.م`,
    ...
    {
      title: 'حد الائتمان',
      value: `${stats.totalCredit.toLocaleString('ar-EG')} ج.م`,
      description: `${creditUsagePercentage}% مستخدم`,
  ...
  const colorClasses = {
    ...
    purple:
      'from-purple-50 to-purple-100 border-purple-200 text-purple-700 bg-purple-200',
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
      {cards.map((card, index) => (
        <div
          key={index}
          className={`bg-gradient-to-r p-6 rounded-xl border ${colorClasses[card.color as keyof typeof colorClasses]}`}
        >
